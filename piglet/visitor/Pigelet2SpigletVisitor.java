...
public class Pigelet2SpigletVisitor extends GJNoArguDepthFirst<MSpiglet> {
    private int tempNum;
    ...
    public MSpiglet visit(Call n) {
        ...
        _ret.setTemp(name);
        // use Temp, not exp
        // or you should let getExp() += exp2 first.
        return _ret;
    }
    ...
    public MSpiglet visit(BinOp n) {
        ...
        if (!exp2.isSimpleExp()) {
            ...
            _ret.setTemp(name);
            // since f1 must be Temp, use Temp, not exp
            // or you should let getExp() += exp1 first.
        } else {
            String name = getNextTemp();
